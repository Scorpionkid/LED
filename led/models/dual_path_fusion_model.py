...
class DualPathFusionModel(RAWBaseModel):
    ...
    def __init__(self, opt):
        super(DualPathFusionModel, self).__init__(opt)
        ...
        self.enable_intermediate_supervision = opt['network_g'].get('enable_intermediate_supervision', False)
    ...
    def optimize_parameters(self, current_iter):
        ...
        if self.use_amp:
            ...
            self.output, texture_mask, detail_output, denoise_output = self.net_g(**inputs)

            loss_dict = OrderedDict()
            l_total = 0

            # 主输出损失 - 使用普通的L1损失
            l_main = self.cri_pix(self.output, self.gt)
            loss_dict['l_main'] = l_main
            l_total += l_main

            # 细节路径中间监督 - 使用感知损失和梯度损失
            if detail_output is not None and self.enable_intermediate_supervision:
                if hasattr(self, 'cri_perceptual') and self.cri_perceptual is not None:
                    l_detail_percep = self.cri_perceptual(detail_output, self.gt)
                    loss_dict['l_detail_percep'] = l_detail_percep
                    l_total += l_detail_percep * 0.1  # 权重可调

                if hasattr(self, 'cri_gradient') and self.cri_gradient is not None:
                    l_detail_grad = self.cri_gradient(detail_output, self.gt)
                    loss_dict['l_detail_grad'] = l_detail_grad
                    l_total += l_detail_grad * 0.5  # 权重可调

            # 降噪路径中间监督 - 使用L1损失
            if denoise_output is not None and self.enable_intermediate_supervision:
                l_denoise = self.cri_pix(denoise_output, self.gt)
                loss_dict['l_denoise'] = l_denoise
                l_total += l_denoise * 0.5  # 权重可调
            ...
            #     # 记录异常情况
            #     if pred_stats['max'] > 100.0 or pred_stats['has_nan'] or pred_stats['has_inf']:
            ...
            if self.detect_gradient_explosion(l_total, current_iter, loss_dict, threshold=1.0):
                self.log_dict = self.reduce_loss_dict(loss_dict)
                return
    ...
    def test(self):
        ...
        if hasattr(self, 'net_g_ema'):
            ...
            with torch.no_grad():
                if self.use_texture_detection:
                    self.output, self.texture_mask, _, _ = self.net_g_ema(**inputs)
                else:
            ...
            with torch.no_grad():
                if self.use_texture_detection:
                    self.output, self.texture_mask, _, _ = self.net_g(**inputs)
                else:
                    self.output, _ = self.net_g(**inputs)
    ...
    def _prepare_inputs(self, lq, noise_map=None, training=True):
        inputs = {'x': lq}
    ...
    def detect_gradient_explosion(self, loss, current_iter, loss_dict, threshold=2.0, skip_update=True):
